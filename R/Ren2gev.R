...
Ren2gev <- function(object,
    ...
    if (abs(parRen["shape"]) > eps) {
        ...
        prov <- 1.0
        parGev[ , "loc"] <- log(parRen["lambda"] * w)
    }
...
gev2Ren <- function(parGev,
    ...
    if (jacobian) {
        ...
        if (fixed == "threshold") {      
            ...
            derParRen["lambda", ] <- -C^(-1/parGev["shape"] - 1) * derC / w /
                parGev["shape"]
            ...
            derParRen["threshold", "shape"] <-
                ( (1 - Le) / parGev["shape"] - log(L) * Le ) * parGev["scale"] /
                    parGev["shape"]
    ...
    if (!is.null(vcovGev)) {
        if (!is.matrix(vcovGev) || (nrow(vcovGev) != 3L) ||
            (ncol(vcovGev) != 3L)) {
